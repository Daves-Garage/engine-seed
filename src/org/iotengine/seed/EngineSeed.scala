...
import akka.stream.actor.{ActorSystem, ActorRefFacotry}
...
object Main extends App {
  ...
  config.getString("example.subscriber") match {
    case "server" => startStreamsServerWithPublsher(publisher, config.getInt("example.server.port"))
...
objectMainFunctions {
  ...
  def startStreamServerWithPublisher(publisher: Publisher[ChunkStreamPart], port: Int)
        (implicit system: ActorSystem, materializer: FlowMaterializer) = { 
          ...
          HttpServer.bindServer(port) {
            ...
            case _: HttpRequest => HttpResonse(404, entity = "Uknown resource!")
          }
  }

  def startPrintSubscriber(publisher: Publisher[ChunkStreamPart], delay: Long)(implicit arf: ActorRefFacotry) = {
